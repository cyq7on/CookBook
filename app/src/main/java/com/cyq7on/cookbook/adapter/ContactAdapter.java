...
import com.cyq7on.cookbook.bean.Friend;
...
public class ContactAdapter extends BaseRecyclerAdapter<Friend> {
    ...
    public void bindView(BaseRecyclerHolder holder, Friend friend, int position) {
        if(holder.layoutId== com.cyq7on.cookbook.R.layout.item_contact){
        ...
        }else if(holder.layoutId== com.cyq7on.cookbook.R.layout.header_new_friend){
            if(NewFriendManager.getInstance(context).hasNewFriendInvitation()){
                holder.setVisible(com.cyq7on.cookbook.R.id.iv_msg_tips,View.VISIBLE);
            }else{
                holder.setVisible(com.cyq7on.cookbook.R.id.iv_msg_tips, View.GONE);
            }
        }
    }
